import { Sequencer } from "../distrib/workers/sequencer.js";
import { assert, asyncPause, syncPause, makeTestCases } from './lib.js';
export let test = makeTestCases();
const kStoppable = true;
const kNonstop = false;
const kInterrupt = true;
const kOrdinary = false;
async function task(id, sequencer, stoppable, interrupt, duration) {
    let success = false;
    let started = 0;
    let finished = 0;
    try {
        console.log('task', id, 'body launch...');
        let body = async (orStop) => {
